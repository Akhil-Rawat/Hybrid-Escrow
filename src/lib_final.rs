...
sol! {
    ...
    event PlanCreated(uint256 indexed planId, address indexed provider, uint256 price, uint256 interval, string metadataHash);
...
impl SubscriptionEscrow {
    ...
    pub fn create_plan(&mut self, price: U256, interval: U256, metadata_hash: String) -> Result<U256, SubscriptionError> {
        let caller = self.vm().msg_sender();
    ...
    pub fn get_plan(&self, plan_id: U256) -> (Address, U256, U256) {
        ...
        (provider, price, interval)
    }

    // Get subscription details by subscription ID
    pub fn get_subscription(&self, subscription_id: U256) -> (U256, Address, U256, bool) {
        let plan_id = self.subscription_plan_id.get(subscription_id);
        let subscriber = self.subscription_subscriber.get(subscription_id);
        let last_payment = self.subscription_last_payment.get(subscription_id);
        let active = self.subscription_active.get(subscription_id);
        (plan_id, subscriber, last_payment, active)
    }

    // Get provider registration status and plan count
    pub fn get_provider_info(&self, provider: Address) -> bool {
        self.registered_providers.get(provider)
    }
